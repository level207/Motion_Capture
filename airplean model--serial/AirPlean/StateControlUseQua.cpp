...
VOID SetWorldMatrix()
{
	...
	*/
	...
	fRoll  = (fRoll_temp  - fPitch_pre) / 5;// 横滚
...
bool Readsscom()
{
	...
	bReadStat = ReadFile(hCom,BUF,6,&wCount,NULL);
	if(!bReadStat)
	{
		//AfxMessageBox("读串口失败!");
		return FALSE;
...
bool Initsscom()
{
	...
	dcb.StopBits=TWOSTOPBITS; //一个停止位
	SetCommState(hCom,&dcb);

	PurgeComm(hCom,PURGE_TXCLEAR|PURGE_RXCLEAR);//清空串口句柄的输入缓冲区和输出缓冲区
...
// Desc: 读串口数据
//-----------------------------------------------------------------------------
DWORD WINAPI thread_read_uart(LPVOID lpParam) 
...
INT WINAPI WinMain( HINSTANCE hInst, HINSTANCE, LPSTR, INT )
{
	...
	hThread=CreateThread(NULL, 0, thread_read_uart, &dwThreadParam, 0, &dwThreadID);
    ...
    { 
        ...
        if( SUCCEEDED( InitGeometry() ) )
        {
            ...
            while( msg.message!=WM_QUIT )
            {
				...
				{
					if ((timeGetTime() - pre_frame_time) > 33)//设置每秒30帧的速率渲染
					{
						Render();  //渲染场景
						pre_frame_time = timeGetTime() - 33;
					}
					/*
					if ((timeGetTime() - pre_second_time) > 100)//每一秒钟更新一下欧拉角，并且把前后两次的欧拉角差值等分为33份来做变换
					{
						fYaw_loop   = fYaw_temp - fYaw_pre;
						fPitch_loop = fPitch_temp - fPitch_pre;
						fRoll_loop  = fRoll_temp - fRoll_pre;

						fYaw_pre   = fYaw_temp;
						fPitch_pre = fPitch_temp;
						fRoll_pre  = fRoll_temp;

						pre_second_time = timeGetTime() - 100;
					}
					*/
				}
            }
